angular.module('deckOfCardApp', [])
    .controller('CardController', function() {
        ...
        vm.drawCard = function() {
            ...
            var suitRandom = Math.floor(Math.random() * 4);
            var rankRandom = Math.floor(Math.random() * 13);
            var card = {
                "suits": vm.suits[suitRandom],
                "rank": vm.ranks[rankRandom]
            };
            vm.cardInDackAvailble = true;
            var cardIndex = vm.suits[suitRandom] + "_" + vm.ranks[rankRandom];
